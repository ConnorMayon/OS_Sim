...
Function Output/Parameters: pointer to config data ponter (ConfigDataType **),
...
Device Output/device: none
Dependencies: copyString, fopen, getSstringToDelimiter, compareString,
...
bool getConfigData( char *fileName,
                          ConfigDataType **configData, char *endStateMsg )
   {
    // initialize function/variables
        ...
        int intData, dataLineCode, dataClearPos, lineCtr = 0;
        double doubleData;
    ...
    *configData = NULL;
    ...
    while( lineCtr < NUM_DATA_LINES )
       {
        // clear dataBuffer
        for ( dataClearPos = MAX_STR_LEN - 1; dataClearPos >= 0; dataClearPos-- )
           dataBuffer[dataClearPos] = NULL_CHAR;
        ...
        // strip trailingspaces of config leader lines
...
Dependencies: compareString
*/
int getDataLineCode( const char *dataBuffer )
   {
...
void stripTrailingSpaces( char *str )
   {
    // initialize index to length of string - 1 to get the highest array index
...
Function Input/Parameters: line code number for specific config value (int)
                           ...
                           string value, as need (const char *)
Function Output/Parameters: none
