...
bool getMetaData( char *fileName, OpCodeType **opCodeDataHead, char *endStateMsg )
   {
    ...
    int accessResult, dataClearPos, startCount = 0, endCount = 0;
    ...
    *opCodeDataHead = NULL;
    ...
    while( accessResult == COMPLETE_OPCMD_FOUND_MSG )
       {
        // Clear buffer; superfluous characters at end causing errors
        for ( dataClearPos = MAX_STR_LEN - 1; dataClearPos >= 0; dataClearPos-- )
            dataBuffer[ dataClearPos ] = NULL_CHAR;
...
int getOpCommand( FILE *filePtr, OpCodeType *inData )
   {
    ...
    for( dataClearPos = STD_STR_LEN; dataClearPos >= 0; dataClearPos--)
        strBuffer[ dataClearPos ] = NULL_CHAR;
